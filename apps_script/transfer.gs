 ...
 * 転記 → 集約 を続けて行う。
 *
 * アンケートの案内はATS側のサンクスメールに記載しているため、
 * こちらからは送っていない。
 * 送信処理（sendFormRequests / sendSurveyFromNotice）は残してあるので、
 * 必要になったら下の steps に1行足せば元に戻る。
 *
 * トリガー設定:
 *   runAll を「時間ベースのタイマー」1時間おき（これ1つでよい）
 */
function runAll() {
  const steps = [
    ['転記', transferAll],
    // ['メール送信', sendFormRequests],   ← こちらから送る場合はこの行を戻す
    ['集約', aggregateAll],
  ];
