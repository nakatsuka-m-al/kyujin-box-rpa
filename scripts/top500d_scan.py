...
OUT_JSON = "/Users/masakatsu/Desktop/kyujin_box_rpa/top500_yuki_companies.json"
MAX_PAGES = 1200
...
def main():
    ...
    with sync_playwright() as p:
        ...
        total = open_sorted_list(page, nationwide=True, category="1", order="2")
        print(f"全国・有料職業紹介: {total:,}事業所 を就職者数(4ヶ月以上)降順で走査", flush=True)
        ...
        while len(companies) < TARGET and page_num <= MAX_PAGES:
            if page_num > 1:
                try:
                    ...
                    try:
                        open_sorted_list(page, nationwide=True, category="1", order="2")
                        goto_page(page, page_num)
    ...
    result = sorted(companies.values(), key=lambda x: -(x["就職者数_4ヶ月以上"] or 0))
    ...
    if result:
        print(f"1位: {result[0]['事業主名称']} ({result[0]["就職者数_4ヶ月以上"]:,}件)", flush=True)
        print(f"{len(result)}位: {result[-1]['事業主名称']} ({result[-1]["就職者数_4ヶ月以上"]:,}件)", flush=True)
